...
class JobViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # Queue the job for async processing with Celery
        from .tasks import process_image_generation
        task = process_image_generation.delay(job.id)
        logger.info(f"Job {job.id} queued for processing (Celery task ID: {task.id})")
